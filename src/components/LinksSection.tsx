...
import { FaSteam, FaYoutube, FaGithub } from "react-icons/fa";
...
export const LinksSection: FC<LinksSectionProps> = ({ lang, openUrl }) => {
    ...
    return (
        <PanelSection title={`🔗 ${t("links", lang)}`}>
            ...
            <div style={{ padding: "8px 16px", background: "rgba(255,255,255,0.03)", borderRadius: "8px", margin: "8px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        ...
                        <span style={{ fontSize: "14px", fontWeight: 700 }}>{lang === "uk" ? "Підтримати на Ko-fi" : "Support on Ko-fi"}</span>
                    </div>
                    <div style={{ display: "flex", gap: "8px" }}>
                        <button onClick={() => openUrl(kofiUrl)} style={{ background: "#FF5E5B", color: "#fff", border: "none", borderRadius: "4px", padding: "4px 12px", fontWeight: 800 }}>Donate</button>
                        <button onClick={() => setShowKofiQR(!showKofiQR)} style={{ background: "rgba(255,255,255,0.1)", color: "#fff", border: "none", borderRadius: "4px", padding: "4px 8px" }}>QR</button>
                    </div>
                </div>
                {showKofiQR && (
            ...
            <div style={{ padding: "8px 16px", background: "rgba(255,255,255,0.03)", borderRadius: "8px", margin: "8px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                        ...
                        }} style={{ background: "#26A17B", color: "#fff", border: "none", borderRadius: "4px", padding: "4px 12px", fontWeight: 800 }}>Donate</button>
                        <button onClick={() => setShowUsdtQR(!showUsdtQR)} style={{ background: "rgba(255,255,255,0.1)", color: "#fff", border: "none", borderRadius: "4px", padding: "4px 8px" }}>QR</button>
                    </div>
                </div>
                {showUsdtQR && (
                    <div style={{ display: "flex", justifyContent: "center", padding: "16px", background: "#fff", borderRadius: "8px", marginTop: "10px" }}>
            ...
            <LinkButton onClick={() => openUrl("https://kuli.com.ua/")} icon={<span>🇺🇦</span>} label="Kuli.com.ua" />
            <LinkButton onClick={() => openUrl("https://www.youtube.com/watch?v=24gxXddKNv0")} icon={<FaYoutube color="#f00" />} label={t("video_guide", lang)} />
            <LinkButton onClick={() => openUrl("https://steamcommunity.com/sharedfiles/filedetails/?id=3137617136")} icon={<FaSteam color="#1b2838" />} label={t("text_guide", lang)} />
