...
export const LinksSection: FC<LinksSectionProps> = ({ lang, openUrl }) => {
    ...
    return (
        <PanelSection title={`🔗 ${t("links", lang)}`}>
            {/* Ko-fi Support */}
            <PanelSectionRow>
                <div style={{ padding: "8px 12px", background: "rgba(255,255,255,0.03)", borderRadius: "8px", width: "100%" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                            <svg height="18" width="18" viewBox="0 0 24 24" fill="#FF5E5B">
                            ...
                            <span style={{ fontSize: "13px", fontWeight: 700 }}>{lang === "uk" ? "Підтримати на Ko-fi" : "Ko-fi Support"}</span>
                        </div>
                    </div>
                    <div style={{ display: "flex", gap: "8px" }}>
                        <button onClick={() => openUrl(kofiUrl)} style={{ flex: 1, background: "#FF5E5B", color: "#fff", border: "none", borderRadius: "4px", padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>Donate</button>
                        <button onClick={() => setShowKofiQR(!showKofiQR)} style={{ background: "rgba(255,255,255,0.1)", color: "#fff", border: "none", borderRadius: "4px", padding: "4px 12px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                            <FaQrcode size={18} />
                        </button>
                    </div>
                    {showKofiQR && (
                        <div style={{ display: "flex", justifyContent: "center", padding: "12px", background: "#fff", borderRadius: "8px", marginTop: "8px" }}>
            ...
            <PanelSectionRow>
                <div style={{ padding: "8px 12px", background: "rgba(255,255,255,0.03)", borderRadius: "8px", width: "100%" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                            <div style={{ background: "#26A17B", borderRadius: "50%", width: "18px", height: "18px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                <span style={{ color: "white", fontSize: "11px", fontWeight: "bold" }}>$</span>
                            </div>
                            <span style={{ fontSize: "13px", fontWeight: 700 }}>USDT (TRC20)</span>
                        </div>
                    </div>
                    <div style={{ display: "flex", gap: "8px" }}>
                        <button onClick={() => setShowUsdtQR(!showUsdtQR)} style={{ flex: 1, background: "rgba(255,255,255,0.1)", color: "#fff", border: "none", borderRadius: "4px", padding: "8px 12px", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", cursor: "pointer" }}>
                            <FaQrcode size={16} />
                            <span style={{ fontSize: "13px" }}>{lang === "uk" ? "Показати QR-код" : "Show QR Code"}</span>
                        </button>
                    </div>
                    {showUsdtQR && (
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "12px", background: "#fff", borderRadius: "8px", marginTop: "8px" }}>
                            <img src={generateQRCode(usdtAddress)} alt="USDT TRC20 QR Code" style={{ width: "140px", height: "140px" }} />
                            <div style={{ marginTop: "8px", color: "#333", fontSize: "9px", wordBreak: "break-all", textAlign: "center", fontWeight: "bold" }}>
                                {usdtAddress}
                            </div>
                        </div>
                    )}
