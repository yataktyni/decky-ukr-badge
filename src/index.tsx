...
async function loadSettings(): Promise<SettingsType> {
    try {
        console.log("[decky-ukr-badge] Loading settings on plugin init...");
        const resp = await callBackend<SettingsType>("get_settings", {});
        currentSettings = { ...DEFAULT_SETTINGS, ...resp };
        console.log("[decky-ukr-badge] Loaded settings:", currentSettings);
            ...
            afterPatch(
                ...
                (_: unknown[], ret: ReactElement) => {
                    try {
                        ...
                        const appId = routeProps.path?.match(
                            /\/library\/app\/(\d+)/,
                        )?.[1];
                        ...
                        console.error(
                            "[decky-ukr-badge] Error in route patch:",
                            e,
                        );
                    }
                    return ret;
